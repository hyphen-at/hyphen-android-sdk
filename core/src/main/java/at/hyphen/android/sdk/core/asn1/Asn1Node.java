...
public final class Asn1Node {
    ...
    public static final class Builder {
        ...
        public Builder addChildAsInteger(int tag, int value) {
            ...
            byte[] dataBytes = Asn1Utils.signedIntToBytes(value);
        ...
        public Builder addChildAsBytesFromHex(int tag, String hex) {
            return addChildAsBytes(tag, Asn1Utils.hexStringToBytes(hex));
        }
        ...
        public Builder addChildAsBits(int tag, int value) {
            ...
            dataBytes[0] = Asn1Utils.countTrailingZeros(dataBytes[dataLength - 1]);
    ...
    private static boolean isConstructedTag(int tag) {
        ...
        byte[] tagBytes = Asn1Utils.unsignedIntToBytes(tag);
    ...
    private static int calculateEncodedBytesNumForLength(int length) {
        ...
        int len = 1;
        if (length > 127) {
            len += Asn1Utils.byteNumForUnsignedInt(length);
        }
        return len;
    }
    ...
    Asn1Node(int tag, @Nullable byte[] src, int offset, int length) {
        ...
        mEncodedLength =
                Asn1Utils.byteNumForUnsignedInt(mTag)
    ...
    public int asInteger() throws InvalidAsn1DataException {
        ...
        try {
            return Asn1Utils.bytesToInt(mDataBytes, mDataOffset, mDataLength);
    ...
    public long asRawLong() throws InvalidAsn1DataException {
        ...
        try {
            return Asn1Utils.bytesToRawLong(mDataBytes, mDataOffset, mDataLength);
        } catch (IllegalArgumentException | IndexOutOfBoundsException e) {
            throw new InvalidAsn1DataException(mTag, "Cannot parse data bytes.", e);
        }
    }
    ...
    public int asBits() throws InvalidAsn1DataException {
        ...
        int bits;
        try {
            bits = Asn1Utils.bytesToInt(mDataBytes, mDataOffset + 1, mDataLength - 1);
        } catch (IllegalArgumentException | IndexOutOfBoundsException e) {
            throw new InvalidAsn1DataException(mTag, "Cannot parse data bytes.", e);
        }
     ...
     */
    public String toHex() {
        return Asn1Utils.bytesToHexString(toBytes());
    }
    ...
    public String getHeadAsHex() {
        String headHex = Asn1Utils.bytesToHexString(Asn1Utils.unsignedIntToBytes(mTag));
        if (mDataLength <= 127) {
            headHex += Asn1Utils.byteToHex((byte) mDataLength);
        } else {
            byte[] lenBytes = Asn1Utils.unsignedIntToBytes(mDataLength);
            headHex += Asn1Utils.byteToHex((byte) (lenBytes.length | 0x80));
            headHex += Asn1Utils.bytesToHexString(lenBytes);
        }
        return headHex;
    }
    ...
    private int write(byte[] dest, int offset) {
        // Writes the tag.
        offset += Asn1Utils.unsignedIntToBytes(mTag, dest, offset);
        // Writes the length.
        if (mDataLength <= 127) {
            dest[offset++] = (byte) mDataLength;
        } else {
            // Bytes required for encoding the length
            int lenLen = Asn1Utils.unsignedIntToBytes(mDataLength, dest, ++offset);
